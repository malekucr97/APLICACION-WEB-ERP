...
@Component({selector: 'app-factores-inferibles-macred',
            templateUrl: './factores-inferibles.component.html',
            styleUrls: [ '../../../../../assets/scss/app.scss' ],
            standalone: false
})
export class FactoresInferiblesComponent extends OnSeguridad implements OnInit {
  
  private nombrePantalla: string = 'factores-inferibles.html';
  ...
  listObjetos: FactoresInferibles[] = [];
  ...
  public objSeleccionado: FactoresInferibles = undefined;
  ...
  registrar() {
    ...
    this.macredService.POST_FACTORES_INFERIBLES(objForm)
      .pipe(first())
      .subscribe((response) => {
  ...
  actualizar() {
    ...
    this.macredService.PUT_FACTORES_INFERIBLES(objForm)
      .pipe(first())
      .subscribe((response) => {
  ...
  eliminar() {
    ...
    this.macredService.DELETE_FACTORES_INFERIBLES(this.objSeleccionado)
      .pipe(first())
      .subscribe((response) => {
  ...
  public seleccionarObjeto(obj: FactoresInferibles) : void {
  ...
  private actualizaLista() {
    this.mostrarLista = false;
    this.macredService.GET_FACTORES_INFERIBLES()
      .pipe(first())
      .subscribe((response) => { 
        if (response) { 
  ...
  private inicializaFormulario(objUpdate : FactoresInferibles = null) : void {
    ...
    if (objUpdate) {
      ...
      this.formObject = this.formBuilder.group({
        ...
        valorMinimo: [objUpdate.valorMinimo, [Validators.required]],
        valorMaximo: [objUpdate.valorMaximo, [Validators.required]],
        quintilUno: [objUpdate.quintilUno],
        quintilDos: [objUpdate.quintilDos],
        quintilTres: [objUpdate.quintilTres],
        quintilCuatro: [objUpdate.quintilCuatro],
        quintilCinco: [objUpdate.quintilCinco],
  ...
  private obtenerDatosFormulario(registra : boolean = true) : FactoresInferibles {
    ...
    let obj = new FactoresInferibles(this.businessObservable.id,
                                    this.moduleObservable.id,
                                    descripcion.value,
                                    valorMinimo.value,
                                    valorMaximo.value,
  ...
  public calculaMontosInferibles() : void {
    ...
    incremento = (valorMaximo.value - valorMinimo.value) / 5 ;

    this.formObject = this.formBuilder.group({
      descripcion: [descripcion.value, [Validators.required, Validators.maxLength(100)]],
      valorMinimo: [valorMinimo.value, [Validators.required]],
      valorMaximo: [valorMaximo.value, [Validators.required]],
      quintilUno: [incremento, [Validators.required]],
      quintilDos: [(incremento * 2), [Validators.required]],
      quintilTres: [(incremento * 3), [Validators.required]],
      quintilCuatro: [(incremento * 4), [Validators.required]],
      quintilCinco: [(incremento * 5), [Validators.required]],
      estado: [estado.value]
    });
  }
}
