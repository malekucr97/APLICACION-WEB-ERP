...
export class Articulo15Component implements OnInit {
    ...
    ngOnInit() {
        ...
        this.cumplimientoService.getArticle15Business(this.userObservable.empresa)
            ...
            error => {
                let message : string = $localize`Problemas al consultar el artículo 15. ' + ${error}`;
    ...
    onDelete(article15:Articulo15) : void {
        ...
        if(confirm($localize`Está seguro que desea eliminar el riesgo de artículo 15  ${article15.descripcion}`)) {
            console.log("pos si");
