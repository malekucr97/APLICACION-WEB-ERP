...
export class AddRoleUserComponent extends OnSeguridad implements OnInit {
  ...
  ngOnInit() {
    ...
    if (pUserId !== administrator.id) {
      this.userToAssign = this.listUserSubject.find(
        (x) => x.identificacion === pUserId
      );
      ...
      this.accountService
        ...
        .subscribe(
          (responseListRole) => {
            ...
            this.listAllRoles.splice(
              this.listAllRoles.findIndex((r) => r.id == administrator.id),
              1
            );

            if (this.userToAssign.idRol) {
              this.isRolAssign = true;
              this.role = this.listAllRoles.find(
                (x) => x.id === this.userToAssign.idRol
              );
            }
          },
          (error) => {
      ...
      let message: string =
        'No es posible actualizar el rol del administrador.';
      ...
      .subscribe(
        (response) => {
          if (response.exito) {
            ...
            this.listUserSubject[
              this.listUserSubject.findIndex(
                (u) => u.id == this.userToAssign.id
              )
            ] = this.userToAssign;
            this.accountService.loadListUsers(this.listUserSubject);
